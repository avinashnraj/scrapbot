...
class Main(object):
    ...
    def update_price_data(self, sym):
        if self.expert[sym].get_price_data() is None:
            new_data = self.api.get_candles(sym, period=g_time_frame[sym],
                                                  number=g_number_of_candles[sym])
            self.expert[sym].set_price_data(new_data)
            Logger.print("Initial Price Data Received...")
            return True
        # Normal operation will update pricedata on first attempt
        new_data = self.api.get_candles(sym, period=g_time_frame[sym], number=g_number_of_candles[sym])
        current_data = self.expert[sym].get_price_data()
        if new_data[-1] != current_data[-1]:
            self.expert[sym].set_price_data(new_data)
            Logger.print("updated prices found..." + str(self.expert[sym].get_price_data()[-1]))
            return True
        return False
