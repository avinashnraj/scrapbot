...
class Expert(object):
    ...
    def engine(self, sym, ask, bid):
        positions = self.api.get_open_positions(symbol=g_symbol[sym])
        orders = self.api.get_orders(symbol=g_symbol[sym])
        ...
        if len_positions == 0 and len_orders == 0:
            ...
            if signal == "Buy":
                ...
                self.api.open_trade(symbol=g_symbol[sym], is_buy=True, rate=ask, amount=g_initial_volume[sym],
                                    tp=tp, sl=sl)
                if g_martingale_enable[sym]:
                    for index in range(1, g_level_maximum[sym]):
                        amt = index * g_scale_factor[sym] * g_initial_volume[sym]
                        self.api.create_entry_order(symbol=g_symbol[sym], is_buy=True, amount=amt,
                                                    tp=tp,
                ...
                self.api.open_trade(symbol=g_symbol[sym], is_buy=False, rate=bid,
                                    amount=g_initial_volume[sym], tp=tp, sl=sl)
                if g_martingale_enable[sym]:
                    for index in range(1, g_level_maximum[sym]):
                        amt = index * g_scale_factor[sym] * g_initial_volume[sym]
                        self.api.create_entry_order(symbol=g_symbol[sym], is_buy=False, amount=amt, tp=tp,
            ...
            if signal == "Close":
                ...
                self.api.close_all_orders(g_symbol[sym])
                ...
                if g_enable_sl_trailing[sym]:
                    if len_orders > 0:
                        self.api.close_all_entry_orders(g_symbol[sym])
        ...
        for row in positions:
            ...
            if row.type == 0:
                if ask > last_close_price and ask - (
                    ...
                    self.api.change_trade_stop_limit(symbol=g_symbol[sym], order_type=row.type,
